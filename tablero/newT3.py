...
def generar_rutas(tablero: tuple, indices: dict, estado_inicial:int, estado_final: int, id_ficha:int, colores_ruta:list, 
    ...
    with open(f"{nombre_arch}_todas.txt", "w") as todas, open(f"{nombre_arch}_ganadorasT2.txt", "w") as ganadoras:
        hito = threading.Thread(target=generar_arboles, args=(colores_ruta, tablero, indices, t, [], estado_final, todas, ganadoras))
        hito.start()
        hito.join()
...
if __name__ == "__main__":
    ...
    inicio = time.time()
    generar_rutas_ficha1 = multiprocessing.Process(target=generar_rutas, args=(tablero, indices, 1, 16, 1, colores_ruta, "f1"))
