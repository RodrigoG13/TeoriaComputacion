class ArbolN_ario:
    ...
    def recorrido_profundidad(self, estados:list, estado_final: int, arbol, ficha:int) -> None:
        ...
        if len(arbol.hijos) == 0:
            with open(f"todas_rutas_f{ficha}.txt", "a+") as rutas:
                estados = [str(num) for num in estados] 
                rutas.write(f"{','.join(estados)}\n")

                if estados[-1] == estado_final:
                    with open(f"rutas_ganadoras_f{ficha}.txt", "a+") as ganadoras:
            ...
            for hijo in arbol.hijos:
                arbol.recorrido_profundidad(estados, estado_final, hijo, ficha)
                estados.pop()
