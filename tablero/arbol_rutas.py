"""
        MÓDULO QUE GENERA LAS RUTAS PARA EL TABLERO MEDIANTE UN ÁRBOL N-ARIO e HILOS
...
def generar_arbol(movimientos: list, tablero: list, indices: dict, arbol, estados: list = [], estado_final: int = None, todas = None, ganadoras = None, lista_estados = [], lista_ganadoras = []) -> None:
        """
        ...
        if arbol.contador != len(movimientos):
            ...
            while len(arbol.hijos) != 0:
                lista_estados, lista_ganadoras = generar_arbol(movimientos, tablero, indices, arbol.hijos[0], estados, estado_final, todas, ganadoras, lista_estados, lista_ganadoras)
                arbol.hijos.pop(0)
                estados.pop()
...
def generar_arboles(movimientos: list, tablero: list, indices: dict, arbol, estados: list = [], estado_final: int = None, todas = None, ganadoras = None, lista_estados = [], lista_ganadoras = []) -> None:
        ...
        for hijo in arbol.hijos:
            ...
            hilo = threading.Thread(target=generar_arbol, args=(movimientos, tablero, indices, hijo, list(estados), 
                                    estado_final, todas, ganadoras, resultados[hijo.estado][0], resultados[hijo.estado][1]))
