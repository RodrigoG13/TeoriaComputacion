...
def generar_rutas(movs_casillas, movimientos, tablero, indices, estado, ruta, estado_final, todas, ganadoras, lista_estados, lista_ganadoras):
    cte = 5
    ...
    if len(ruta) <= len(movimientos):
        ...
        for hijo in hijos:
            ...
            generar_rutas(movs_casillas, movimientos, tablero, indices, hijo, ruta + [hijo], estado_final, todas, ganadoras, lista_estados, lista_ganadoras)
...
def threaded_generar_rutas(movimientos, tablero, indices, estado_inicial, estado_final, todas, ganadoras, movs_casillas):
        ruta = [estado_inicial]
        hilos = {}
        resultados = {}
        ...
        for hijo in hijos_iniciales:
            ...
            with variable_lock:
                crear_bifu(movs_casillas, len(ruta), ruta[-1])
                movs_casillas[len(ruta)][ruta[-1]].add(int(hijo))
            hilo = threading.Thread(target=generar_rutas, args=(movs_casillas, movimientos, tablero, indices, hijo, ruta+[hijo], estado_final, todas, ganadoras, resultados[hijo][0], resultados[hijo][1]))
            hilos[hijo] = hilo
            hilo.start()
...
if __name__ == "__main__":
    ...
    with open("todas.txt", "w") as todas, open("ganadoras.txt", "w") as ganadoras:
        ...
        resultados = threaded_generar_rutas(ruta_colores, tablero, indices, 1, 9, todas, ganadoras, movs_casillas)
