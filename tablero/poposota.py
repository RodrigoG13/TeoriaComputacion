...
# menos escrituras
    ...
    def worker(tablero, indices, movimientos, estado_inicial, estado_final, todas, ganadoras):
        t = ArbolN_ario(estado_inicial, 0)
        estados, lganadoras = t.generar_arbol(movimientos, tablero, indices, t, [], estado_final, todas, ganadoras)
        
        # Sincronización de escritura en archivos usando locks
        todas_lock.acquire()
        todas.write("\n".join(estados))
        todas.write("\n")
        todas_lock.release()

        ganadoras_lock.acquire()
        ganadoras.write("\n".join(lganadoras))
        ganadoras.write("\n")
        ganadoras_lock.release()
...
if __name__ == "__main__":
    ...
    # Crear threads para cada estado inicial
    for estado_inicial in indices:
        th = threading.Thread(target=worker, args=(tablero, indices, ruta, estado_inicial, "9", todas, ganadoras))
        threads.append(th)
        th.start()
