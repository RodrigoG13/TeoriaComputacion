...
class ParticionUniverso:
    ...
    def __init__(self, tam_particion: int, nombre_arch="particionUniverso") -> None:
        ...
        self.arch_univ = open(f"{nombre_arch}.txt", "a+", encoding="utf-8")
        self.tam_particion = tam_particion
        self.arch_indices = open(f"indices.txt", "a+")
    ...
    def obtener_datos_conjuntos(self) -> tuple:
        ...
        """
        if os.path.getsize("indices.txt") == 0:
            self.arch_univ.write("Σ = {ε, 0, 1, ")
            self.arch_indices.write("1")
            return 1
    ...
    def generar_particion_universo(self) -> None:
        ...
        long = self.obtener_datos_conjuntos()
        ...
        for i in range(1, n+1):
            for binary_word in self.binary_combinations(n):
                self.arch_univ.write(f"{binary_word}, ")
            print(i)
